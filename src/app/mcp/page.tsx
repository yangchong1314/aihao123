...
import { mcpSkills, mcpSkillCategories, McpSkill } from "@/data/mcp";
...
export default function McpPage() {
  ...
  const filteredSkills = useMemo(() => {
    const filtered = mcpSkills.filter((skill: McpSkill) => {
  ...
  const hotSkills = useMemo(() => mcpSkills.filter((s) => s.isHot).slice(0, 6), []);
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <nav className="hidden sm:flex items-center gap-6 text-sm text-gray-600">
            ...
            <a href="/mcp" className="text-purple-600 font-medium">MCP</a>
      ...
      <section className="bg-gradient-to-b from-blue-50 to-gray-50 py-12 sm:py-16">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-900 mb-4">MCP 服务器</h2>
          <p className="text-lg text-gray-600 mb-6 max-w-2xl mx-auto">
            290+ MCP 服务器，让你的 AI 安全连接外部工具
          </p>
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-100 text-blue-700 rounded-full text-sm">
            <span>🔌</span>
            <span>Model Context Protocol - 安全、标准化的 AI 工具连接</span>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-gray-900">
              全部 MCP 服务器 ({filteredSkills.length})
            </h3>
            {totalPages > 1 && (
          ...
          {paginatedSkills.length === 0 && (
            <div className="text-center py-12 text-gray-500">
              <p className="text-lg">没有找到匹配的MCP服务器</p>
            ...
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    <span className="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded-full">{selectedSkill.downloads} 下载</span>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {selectedSkill.tags.map((tag) => (
                  <span key={tag} className="text-xs px-3 py-1 bg-blue-100 text-blue-600 rounded-full">{tag}</span>
                ))}
              </div>
              ...
              <div className="mt-4 text-sm text-gray-500">作者: {selectedSkill.author}</div>
            </div>
          </div>
        </div>
      )}
...
function SkillCard({ skill, onClick }: { skill: McpSkill; onClick: () => void }) {
  return (
    <div onClick={onClick} className="group block bg-white rounded-xl border border-gray-200 p-5 hover:shadow-lg hover:border-blue-300 transition-all duration-200 cursor-pointer">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">{skill.name}</h4>
